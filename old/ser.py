...
def SER(dTree, node, X_target_node, y_target_node, original_ser=True, no_red_on_cl=False,
    ...
    if original_ser:
        new_node_left, bool_no_red_l = SER(dTree, dTree.tree_.children_left[node], X_target_node_left, y_target_node_left,
                                           original_ser=True)
        node, b = lib_tree.find_parent(dTree, new_node_left)

        new_node_right, bool_no_red_r = SER(dTree, dTree.tree_.children_right[node], X_target_node_right, y_target_node_right,
                                            original_ser=True)
        node, b = lib_tree.find_parent(dTree, new_node_right)

    else:
        new_node_left, bool_no_red_l = SER(dTree, dTree.tree_.children_left[node], X_target_node_left, y_target_node_left, original_ser=False,
        ...
        new_node_right, bool_no_red_r = SER(dTree, dTree.tree_.children_right[node], X_target_node_right, y_target_node_right, original_ser=False,
                                            no_red_on_cl=no_red_on_cl, cl_no_red=cl_no_red,
                                            no_ext_on_cl=no_ext_on_cl, cl_no_ext=cl_no_ext, leaf_loss_quantify=leaf_loss_quantify,
                                            leaf_loss_threshold=leaf_loss_threshold, coeffs=coeffs, root_source_values=root_source_values, Nkmin=Nkmin)
...
def SER_RF(random_forest, X_target, y_target, original_ser=True, bootstrap_=False,
    ...
    for i, dtree in enumerate(rf_ser.estimators_):
        ...
        SER(rf_ser.estimators_[i], 0, X_target[inds], y_target[inds], original_ser=original_ser,
            no_red_on_cl=no_red_on_cl, cl_no_red=cl_no_red,
